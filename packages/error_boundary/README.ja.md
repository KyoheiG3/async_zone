...
`resetKeys` 内のいずれかの値が前回のレンダリングと異なる場合（等価性で比較）、エラー状態がクリアされ `onReset` が発火します。

### 入れ子のバウンダリー

`ErrorBoundary` を入れ子にすると、フォールバックから throw されたエラーは外側の `ErrorBoundary` にエスカレートします（React の error boundary の意味論に準じる）。対象は：
