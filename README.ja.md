...
| [`async_error_boundary`](packages/async_error_boundary)           | 高レベルな `ErrorBoundary` ウィジェット — `builder(context, error, reset)`、`onError` / `onReset` コールバック、`resetKeys` による自動リセット、手動の `showBoundary`。      |
| [`async_transition_boundary`](packages/async_transition_boundary) | React の `useTransition` スタイルのトランジション。サブツリーを `TransitionBoundary` でラップすると、配下の suspend が外側の `AsyncZone` fallback を出さずに吸収されます。   |
...
必要なものだけ入れてください。多くのアプリは、まず `async_zone` + `async_error_boundary` から始めるのがおすすめです：
...
| すぐ使える `ErrorBoundary` ウィジェット（自前で `ErrorZoneWidget` を書くより推奨） | `async_zone` + `async_error_boundary`                                   |
| React `useTransition` スタイルの「新しい state が suspend する間も前の UI を残す」 | `+ async_transition_boundary`                                           |
