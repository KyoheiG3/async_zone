...
A Flutter sample app that demonstrates `async_zone`, `hooks_async_zone`, `async_error_boundary`, and `async_transition_boundary` working together — a React-style Suspense + `use()` + `useTransition` pattern, without any external state-management library.
